...
class OTPFormStudent extends StatefulWidget {
  const OTPFormStudent({
    super.key,
    required this.fem,
    required this.hem,
  ...
  @override
  State<OTPFormStudent> createState() => _OTPFormState();
}

class _OTPFormState extends State<OTPFormStudent> {
...
Widget _buildButton() {
  ...
  Future<void> _submitForm(BuildContext context, TextEditingController pinController) async {
    final stu = await AuthenLocalDataSource.getStudent();
    
    context.read<VerificationCubit>().verifyStudent(stu!.id, widget.email, pinController.text);
  }
